<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="ReporteDenunciaCerrada.aspx.cs" Inherits="Portal_Investigadores.ReporteDenunciaCerrada" %>
    ...
    <form runat="server">
        ...
        <script type="text/javascript">
           ...
           <%-- var tipoLectura = '<%= Session["tipoLectura"] %>';  --%>
            ...
            function cargarDocumentosCerradas() {

                var id = $('#contenido_txtFolio').val();

                console.log(id);

                $.ajax({
                    type: "POST",
                    url: 'ReporteDenunciaCerrada.aspx/CargarSoportesCerradas',
                        ...
                        if (objdata.PR_DocumentosInvestigacion.length > 1) {
                            ...
                            for (i = 0; i < objdata.PR_DocumentosInvestigacion.length - 1; i++) {
                                ...
                                if ($tableID.find('tbody').length > 0) {
                                        ...
                                        $('#docInv tbody').append(newTr);
                                }
                            }
                        } else {
                            $('#docInv').hide();
                            $('#collapseDocsInv .card-body').append('<td><p style="margin-top: 10px;font-size: 1rem; float: left;"> ' + tagsTable.filter(function (tag) { return tag.id == 95; })[0].tag + '</p></td><td></td>');
                        }
                    },
                    error: function (e) {
                        console.log(e);
                    ...
                    <div id="collapseTwo" class="" aria-labelledby="headingTwo"> <%--data-parent="#accordionExample"--%>
                        <div class="card-body">
                            ...
                            <div class="accordion" id="accordionEnt" style="margin-top: 15px;">
                             ...
                             <%--Accordion de Documentos--%>
                            <div class="accordion" id="accordionDocsInv" style="margin-top: 15px;">
                                    <div class="card">
                                        <div class="card-header cardheader-detalle" id="headingDocsInv">
                                            <h2 class="mb-0" style="float: left; width: 94%;">
                                                <button class="btn btn-collapse" type="button" data-toggle="collapse" data-target="#collapseDocs" aria-expanded="false" aria-controls="collapseDocs">
                                                    <% row = tags.Select("id = '38'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Documentos--%>
                                                </button>
                                        ...
                                        <div id="collapseDocsInv" class="" aria-labelledby="headingDocs" >
                                            <div class="card-body">
                                                <div id="tableDocsInv" class="table-editable">
                                                    <%--<span class="table-add float-right mb-3 mr-2 card-add"><a href="#!" style="font-size:30px; font-weight:bold; text-decoration:none;" class="text-success">+</a></span>--%>
                                                    <table id="docInv" class="table table-bordered table-responsive-md table-striped text-center tblinvolucrados">
                                                        <thead>
                                                            <tr>
                                                                <th class="text-left"><% row = tags.Select("id = '39'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Documento--%></th>
                                                            </tr>
                            ...
                            <div class="form-row hidden-print">
                                <div class="form-group col-md-2 ml-auto" id="Div1" runat="server" >
                                    <button type="button" class="btn btn-secondary hidden-print" onclick="location.href='DenunciasCerradas.aspx';" > <% row = tags.Select("id = '67'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Regresar--%></button>
                                </div>
