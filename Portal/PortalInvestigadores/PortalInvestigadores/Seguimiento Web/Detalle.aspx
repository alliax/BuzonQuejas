<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="Detalle.aspx.cs" Inherits="Portal_Investigadores.Detalle" %>
                    ...
                    <div id="collapseOne" class="collapse show" aria-labelledby="headingOne"> <%--data-parent="#accordionExample"--%>
                        <div class="card-body">
                            <form>
                                ...
                                <%--Accordion de Antecedentes--%>
                                <div class="accordion" id="accordionAnte" style="margin-top: 15px;">
                                    <div class="card">
                                        <div class="card-header cardheader-detalle" id="headingAnte">
                                            <h2 class="mb-0" style="float: left; width: 94%;">
                                                <button class="btn btn-link btn-collapse" type="button" data-toggle="collapse" data-target="#collapseAnte" aria-expanded="false" aria-controls="collapseAnte">
                                                    <% row = tags.Select("id = '121'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Denuncias Antecedente--%>
                                                </button>
                                            </h2>
                                            <img src="img/down-arrow.svg" style="float:right; margin-right: 20px; margin-top: 9px; height: 24px; cursor: pointer;" data-toggle="collapse" data-target="#collapseAnte"/>
                                        </div>

                                        <div id="collapseAnte" class="collapse" aria-labelledby="headingAsoc" >
                                            <div class="card-body">
                                                <div id="tableAnte" class="table-editable">
                                                    ...
                                                    <table class="table table-bordered table-responsive-md table-striped text-center tblinvolucrados">
                                                        <thead>
                                                            <tr>
                                                                <th class="text-left"><% row = tags.Select("id = '37'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Id--%></th>
                                                                <th class="text-left"><% row = tags.Select("id = '23'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Título--%></th>
                                                                <th class="text-left"><% row = tags.Select("id = '24'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Grupo--%></th>
                                                                <th class="text-left"><% row = tags.Select("id = '30'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Involucrados--%></th>
                                                                <th class="text-left"><% row = tags.Select("id = '82'");  if (row.Length > 0){Response.Write(row[0][1]);} %> <%--Madurez--%></th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                        </tbody>
