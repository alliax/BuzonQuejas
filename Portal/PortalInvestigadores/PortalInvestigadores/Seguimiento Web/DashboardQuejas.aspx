    ...
    if (idRol == 2 ) { //Vobo 
        ...
        $("#btn1").click(
            function () {
                ...
                for (i = 0; i <= msgData.length-1; i++) {
                    $("#tbl").append('<tr>' + '<td>' + msgData[i].IdQueja + '</td>' + '<td>' + msgData[i].Titulo + '</td>' + '<td>' + msgData[i].Empresa + '</td>' + '<td>' + msgData[i].Sitio + '</td>' + '<td><button type="button" onclick="changeUrl(1,'+ msgData[i].IdMensaje +')" class="btn btn-primary">Detalle</button></td>' + '</tr>');
                }
            }
        );
...
function dashboardData(Opt, idBQ) {
    ...
    function changeUrl(Opt,Id) {
        if (Opt == 1) {
            var url = window.location.origin +'/AltaMensaje?IdMensaje='+Id+'.aspx';
